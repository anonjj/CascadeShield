...
* **Mechanics, function by function:**
  ...
  * `generate_load()` — `requests_count` requests through a `concurrency`-wide `ThreadPoolExecutor` at `interval_s` submission pacing (defaults 50 / 5 / 50ms; the fault phase overrides all three from `compute_load_plan()`); per-request latency and dispatch timestamps captured under a `threading.Lock`; computes TPS, error %, mean latency, and the achieved arrival rate `lambda_achieved`/`lambda_cv`. (TPS is measured but deliberately not reported — see §4.7 and decision D20.)
  ...
  * `log_results()` — appends to `data/master_dataset.csv` (the 36-column `DATASET_HEADERS` schema: identity and provenance, the 5 swept params, the timing DVs, the containment and error columns, the arrival-rate columns, and `excluded_reason`), writing the header only on first creation. Append-only: a crashed sweep loses no completed rows and can be resumed.
...
1. **Throughput is measured, but not reported (D20).** The pacing-overhead defect this entry used to describe — the subtraction over-correcting under fast-fail and inflating TPS — was fixed in `508575f` (2026-06-18): `generate_load` now derives its window from a true last-completion timestamp stamped inside `send_request` under the existing lock, and every row of the live dataset postdates that fix. The *reporting* problem is separate and unfixed: `throughput_loss` divides the fault-phase TPS against a fixed 20-request pre-fault baseline, while the fault phase's `requests_count`/`concurrency`/`interval_s` come from `compute_load_plan()` — sized from the sliding-window type, window size and wait duration, i.e. the independent variables. The measurement window therefore varies with the thing under comparison, and only the ratio is written to the schema, so no collected row can be corrected after the fact. **No TPS-derived number appears in the paper**; the column is retained for reference only. See decision **D20** and `docs/paper/hypotheses.md` §7.
