...
| Statistical treatment (D19) | ✅ **Defined** — Mann-Whitney + Cliff's δ, bootstrap CIs, censoring as rate + conditional timing. **Two deviations closed 2026-09-19** (§5.2: `compare_censored_groups`'s significance test is now cluster-aware — `cluster_permutation_rank_test`, `analysis/exact_tests.py`; verified by re-running `window_type_recovery_leak.py` before/after, one real crossing found and documented, no published number affected. §5.2 addendum: `window_type_recovery_leak.py` now tests each DV once across all $D_w$ — `stratified_cluster_permutation_rank_test` — instead of once per bucket; diffed, no verdict changed, new safe-to-quote number: PRECISE `half_open_to_closed`'s joint test = p=0.025, the exact floor). **One still open** (§5.1: `canary_readout.py`'s H1 still uses Welch's t, backs an already-closed D-004 gate, migration deliberately deferred as its own reviewed step) | — |
