...
| D13 / H3 (recovery leak) | ✅ **Confirmed** — `LEAK_CONFIRMED_ON_HALF_OPEN_LEG`, 36/36 recovered on both arms at all three D_w after the poll-until-transition fix (D21); KM computed on 34/36 (2 host-sleep-corrupted durations excluded), significant everywhere (p=0.0014 at D_w=5/15, p=0.0005 at D_w=30). *Superseded by decision-log.md D26 (2026-09-22): these figures are gateway-contaminated (D23) and retracted — do not quote them. Gateway-verified post-D25 re-collection: COUNT faster than TIME in every stratum, KM ratios 3.51×/2.15×/2.07× at D_w=5/15/30, p at the exact design floor 5.83e-06 (saturated). Branch `h3-evidence-audit`, not yet on `main`.* | — |
...
| **H6** | A uniform edge breaker suppresses interior breaker engagement (gateway shadowing) | 🟡 **"Untestable" verdict corrected 2026-09-17 (D23), not yet re-decided.** The `measurement-plane` isolation was believed complete (`cb_state_pre` "CLOSED at load start" was the evidence — a pre-load snapshot, never evidence for mid-run state). Live-verified it's incomplete: gateway's breaker never sets `sliding-window-size`/`sliding-window-type`, which silently track the swept `default` values, so — combined with D18's confirmed mechanism — gateway's breaker **does** trip under COUNT_BASED at `wait_duration ∈ {15,30}` (20 real trips across 5 experiment_ids in `data/cb_transitions.jsonl`; 0 under TIME_BASED, where the isolation genuinely holds). H6 may be directly testable now, not just via deliberate reconstruction — testability verdict needs re-deciding, not done yet. **Gap fixed 2026-09-18 (D25):** `measurement-plane` now pins every field explicitly (`TIME_BASED`, window 600s, `minimum-number-of-calls=1000000`, genuinely unreachable) — live-verified via a real fault run (`canary`, `T70-W20-D30`, 17/60 failures): zero gateway `CLOSED_TO_OPEN` events. This closes the leak for **any data collected from here on**; the 20 historical trips this row describes are unaffected, still real, still D23/D24's finding | D23, D25, hypotheses.md §7 |
...
*(Superseded by decision-log.md D26, 2026-09-22: computed on gateway-contaminated data (D23).
Do not quote.)*
...
against.
*(Superseded by decision-log.md D26, 2026-09-22: the p-values, ratios and medians in this
paragraph (p=0.0014/0.0005; 9.49×/2.16×/2.40×; COUNT 2.04/9.83/14.99s) were computed on
