...
audits/.cache/content/ (the raw scraped YAML) is gitignored, not committed: it's third-party
code, not ours to republish, and GitHub's push protection caught real leaked secrets in it on
the first attempt to commit this (an API key, OAuth credentials). Only hits.jsonl/repos.jsonl
(metadata -- repo names, paths, blob shas, star counts, no file content) are tracked. Re-running
extract() from scratch requires the local (uncommitted) cache; without it, main() falls back to
the already-extracted analysis/out/lambda_star_instances.csv, so the figure and JSON stay
...
def main():
    ...
    if (AUDIT_DIR / "content").is_dir():
        ...
        df.to_csv(instances_csv, index=False)
    elif instances_csv.exists():
        # audits/.cache/content/ is gitignored (raw third-party YAML, see module docstring) --
        # a fresh checkout re-derives the figure/JSON from the already-extracted CSV instead
        # of failing outright.
        print("audits/.cache/content/ not present locally -- reusing the already-extracted "
        ...
        raise RuntimeError(
            "Neither audits/.cache/content/ (raw scrape, gitignored -- see module docstring) "
            "nor {} (committed extraction) is present. Nothing to build the figure from."
    ...
    file_vals = df.groupby("sha").first()["lambda_star"].tolist()
    repo_vals = df.groupby("repo_full_name").first()["lambda_star"].tolist()

    # The estimand choice is NOT cosmetic -- see estimand_sensitivity_warning below, computed
    # from these four, for how much the median actually moves. Every plausible grouping is
    # reported, not just the two plotted curves, so a reader can see the sensitivity rather
    # than take one number on faith.
    grouping_sensitivity = {
        "instance_level": _quartiles(instance_vals),
        "file_level_one_row_per_scraped_file": _quartiles(file_vals),
        "repo_level_one_row_per_repo": _quartiles(repo_vals),
