...
docs/paper/decision-log.md D-00X entry by itself, though it backs decision D-007.)
...
threats-to-validity (D-007), not fixed.
...
LINEAR. That is the tension D-007 states explicitly rather than leaving for a reviewer.
...
def order_leg_stats(df):
    """Everything D-007 cites, in one place, from real data -- no number typed by hand."""
    df = df.copy()
