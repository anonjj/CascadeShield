...
re-sweep) before treating D13 as closed.**
...
- **$B$/$B_{\text{real}}$ are retired, not fixed (D15).** After three node-set redefinitions
...
- **The FAN_OUT sweep that closes the bullet above compounds with a second confound
  ...
  measurable, and D16 sets the rule that holds **starting now, independent of whether the
