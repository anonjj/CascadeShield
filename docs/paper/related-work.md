...
the record of what was decided from it. **BibTeX for everything cited here, verified against
each landing page (T2, 2026-09-21/22), lives in [`references.bib`](references.bib)** (16
entries) — including broader microservice-resilience/circuit-breaker context citations beyond
this file's own near-miss table, the fix for one author-order error this pass caught (the
"Zdun et al." row below; verified order is El Malki, Zdun, Pautasso), and one near-miss
(Pashko et al.) kept in the table below as a record of what the search found but dropped from
the citable bibliography — no verifiable DOI/volume/pages, and not load-bearing for the verdict.
