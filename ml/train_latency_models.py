...
Retargets the ML layer from the degenerate safe/unsafe blast_radius classifier to
two regressors on the outcomes that actually vary:

    time_to_open     -- detection latency  (driven by window_type/size/threshold)
    time_to_recover  -- recovery latency   (driven by wait_duration)

Key methodological point: the sweep is 54 unique configs x 3 replicates, and
experiment_id is unique per CONFIG. A random train/test split would put replicates
of the same config on both sides and inflate scores. All CV here is GroupKFold
grouped by experiment_id, so effective n = 54 configs, not 156 rows.

Usage:
    python train_latency_models.py [path/to/dataset.csv]
"""
import sys
...
CANDIDATES = [
    ...
    "data/master_dataset_v2_latency_5svc.csv",  # post Task-0 archive name
]
FEATURES = ["window_type_is_time", "threshold", "window_size", "wait_duration"]
RECOVER_OUTLIER_S = 1000.0  # one known harness hang at 7540s; all real rows <= 65.7s
...
def recommend(tree, target_seconds, top_k=5):
    """Enumerate the swept config grid, predict latency, return configs closest to target."""
    grid = [
...
def descriptives(df):
    print(f"\n{'=' * 68}\nDESCRIPTIVES (for the paper's results table)\n{'=' * 68}")
